...
const parsing = async (page) => {
    ...
    $recruit.each((idx, node) => {
        const title = $(node).find('.job_tit > a:eq(0)').text().trim()
        const company = $(node).find('.corp_name > a:eq(0)').text().trim()
        const date = $(node).find('.job_date >span:eq(0)').text().trim()
        const location = $(node).find('.job_condition > span > a:eq(1)').text().trim()
        const url1 = 'https://www.saramin.co.kr' + $(node).find('.corp_name > a:eq(0)').attr("href");
        const url2 = 'https://www.saramin.co.kr' + $(node).find('.job_tit > a:eq(0)').attr("href");
        
        recruits.push({
            title, company, date, location, url1, url2
        })    
    });
